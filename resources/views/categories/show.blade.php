...
<div class="container-custom px-4 sm:px-6 lg:px-8 pb-16">
    ...
    @if($category->description)<p class="text-surface-600 mb-4">{{ $category->description }}</p>@endif

    @if($category->children->count())
    <div class="flex flex-wrap gap-3 mb-8">
        @foreach($category->children as $child)
        <a href="{{ route('categories.show', $child) }}" class="px-4 py-2 bg-surface-100 hover:bg-primary-50 text-surface-700 hover:text-primary-600 rounded-lg text-sm font-medium transition-colors">{{ $child->name }}</a>
        @endforeach
    </div>
    @endif
            ...
            <div id="products-grid" class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6 items-start">
                ...
                @empty
                    <div class="col-span-full text-center py-16">
                        <p class="text-surface-500 text-lg">No products in this category yet.</p>
                    </div>
                @endforelse
            </div>
...
(function () {
    ...
    function loadNextPage(sentinel) {
        ...
        .catch(function () { scrollLoading = false; });
    }
    ...
    function rebind() {
        ...
        if (resetLink) {
            resetLink.addEventListener('click', function (e) {
                ...
                fetchController = new AbortController();
                loading.style.display = 'flex';
                fetch(action, { headers: { 'X-Requested-With': 'XMLHttpRequest' }, signal: fetchController.signal })
                .then(function (res) { return res.text(); })
                .then(function (html) {
                    var doc = new DOMParser().parseFromString(html, 'text/html');
